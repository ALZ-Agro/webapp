 (function(){
	...
	.provider('ngImageGalleryOpts', function(){
		var defOpts = {
			...
			piracy 			: 	false,
			imgAnim 		: 	'fadeup',
			errorPlaceHolder:   '¡Error cargando la imágen!'
		};
	...
	function($rootScope, $timeout, $q, ngImageGalleryOpts){
		return {
            ...
            template: '<div class="ng-image-gallery img-move-dir-{{_imgMoveDirection}}" ng-class="{inline:inline,trasparent:forceModal}" ng-hide="images.length == 0">'+
								...
								'<div class="ng-image-gallery-content" ng-show="!imgLoading" ng-click="backgroundClose($event);">'+
									...
									'<div class="galleria">'+
										...
										'<div class="galleria-images img-anim-{{imgAnim}} img-move-dir-{{_imgMoveDirection}}">'+
											'<img class="galleria-image" ng-right-click ng-repeat="image in images track by image.id" ng-if="_activeImg == image" ng-src="{{image.url}}" ondragstart="return false;" ng-click="methods.openModalForInlineGallery($index)" ng-attr-alt="{{image.alt || undefined}}"/>'+
										'</div>'+
			...
			link : {
				pre : function(scope, elem, attr, rootScope){
					...
					**/
                    ...
                    if (scope.inline) {
                        scope.$on('disableForceModal', function () {
                            scope.forceModal = false;
                        });

                    }
                   
                    scope.$on('set-modal-index', function (event, index) {
                        scope._activeImageIndex = (index) ? index : 0;
                        scope.opened = true;
                    });


                    //if (scope.inline) {
                    //    scope.$on('syncIndex', function (event, index) {
                    //        scope._activeImageIndex = (index) ? index : 0;
                    //    });
                    //}
                  

                    scope.methods.openModalForInlineGallery = function (imgIndex) {
                        scope.forceModal = true;
                        $rootScope.$broadcast('image-opened', imgIndex);
                    }
					...
					scope.methods.close = function(){
						scope.opened = false; // Model closed
                            ...
                            $rootScope.$broadcast('disableForceModal');
					...
					scope.methods.next = function(){
                        ...
                        //if (!scope.inline) {
                        //    $rootScope.$broadcast('syncIndex', scope._activeImageIndex);
                        //}
					}
