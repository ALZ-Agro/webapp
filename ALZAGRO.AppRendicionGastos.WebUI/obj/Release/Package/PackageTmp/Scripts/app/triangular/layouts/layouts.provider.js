(function() {
    ...
    function layoutProvider() {
        var layoutDefaults = {
            ...
            showToolbar: true,
            footer: true,
            contentTemplateUrl: '../Scripts/app/triangular/layouts/default/default-content.tmpl.html',
            sidebarLeftTemplateUrl: '../Scripts/app/layouts/leftsidenav/leftsidenav.tmpl.html',
            sidebarLeftController: 'MenuController',
            sidebarRightTemplateUrl: '../Scripts/app/triangular/components/notifications-panel/notifications-panel.tmpl.html',
            sidebarRightController: 'NotificationsPanelController',
            toolbarTemplateUrl: '../Scripts/app/triangular/components/toolbars/toolbar.tmpl.html',
            toolbarController: 'DefaultToolbarController',
            footerTemplateUrl: '../Scripts/app/triangular/components/footer/footer.tmpl.html'
        };
        ...
        this.$get = function() {
            ...
            function updateLayoutFromState(event, toState) {
                ...
                angular.forEach(resetableOptions, function (option) {
                    if (option != "sideMenuSize") {
                        layout[option] = layoutDefaults[option];
                    } else {
                        if (layout[option] === "icon") {
                            /// Find sidenav and remove hover class, to reset navbar collapsed state
                            var sidenav = document.getElementsByClassName('triangular-sidenav-left');
                            if (sidenav && sidenav[0]) {
                                sidenav[0].classList.remove('hover');
                            }
                        }
                        if (layout[option] === "off" || layout[option] === "hidden") {
                            layout[option] = layoutDefaults[option];
                        }
                    }
                });
